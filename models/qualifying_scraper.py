...
Supports both standard Qualifying and Sprint Qualifying sessions.
"""
...
class QualifyingScraper:
    ...
    SOURCES = {
        ...
        'formula1': ScrapingSource(
            ...
            # Supports both /qualifying and /sprint-qualifying paths.
            # {session_slug} is either 'qualifying' or 'sprint-qualifying'.
            pattern='/en/results/2026/races/{race_id}/{track_slug}/{session_slug}',
            selectors={
    ...
    # Map internal track names to (race_id, url_slug) tuples for F1.com.
    # race_id values sourced from https://www.formula1.com/en/results/2026/races
    TRACK_NAME_MAPPING = {
        'albert park circuit':            ('1279', 'australia'),
        'shanghai international circuit': ('1280', 'china'),
        'suzuka circuit':                 ('1281', 'japan'),
        'bahrain international circuit':  ('1282', 'bahrain'),
        'jeddah corniche circuit':        ('1283', 'saudi-arabia'),
        'miami international autodrome':  ('1284', 'miami'),
        'circuit gilles villeneuve':      ('1285', 'canada'),
        'circuit de monaco':              ('1286', 'monaco'),
        'circuit de barcelona-catalunya': ('1287', 'spain'),
        'red bull ring':                  ('1288', 'austria'),
        'silverstone circuit':            ('1289', 'great-britain'),
        'circuit de spa-francorchamps':   ('1290', 'belgium'),
        'hungaroring':                    ('1291', 'hungary'),
        'circuit zandvoort':              ('1292', 'netherlands'),
        'autodromo nazionale monza':      ('1293', 'italy'),
        'madrid street circuit':          ('1294', 'spain'),
        'baku city circuit':              ('1295', 'azerbaijan'),
        'marina bay street circuit':      ('1296', 'singapore'),
        'circuit of the americas':        ('1297', 'united-states'),
        'autodromo hermanos rodriguez':   ('1298', 'mexico'),
        'autodromo jose carlos pace':     ('1299', 'brazil'),
        'las vegas strip circuit':        ('1300', 'las-vegas'),
        'losail international circuit':   ('1301', 'qatar'),
        'yas marina circuit':             ('1302', 'abu-dhabi'),
    }

    # Tracks that have a Sprint Qualifying session in 2026
    SPRINT_QUALIFYING_TRACKS = {
    ...
    # Legacy slug mapping used by racingnews365 / motorsport.com
    TRACK_SLUG_MAPPING = {
    ...
    def scrape_qualifying_results(
        self,
        track_name: str,
        source: str = 'racingnews365',
        session_type: str = 'auto',
    ) -> Optional[List[QualifyingResult]]:
        """
        Scrape qualifying results from specified source.

        Args:
            track_name: Internal track name (e.g. 'Shanghai International Circuit').
            source: One of 'racingnews365', 'formula1', 'motorsport'.
            session_type: 'qualifying', 'sprint-qualifying', or 'auto' (auto-detect
                          based on the track's weekend format).
        """
        ...
        url = self._build_url(track_name, source_config, session_type=session_type)
    ...
    def _build_url(
        ...
        Args:
            ...
            source: ScrapingSource config.
            session_type: 'qualifying', 'sprint-qualifying', or 'auto'.
        """
        track_key = track_name.lower()

        # Resolve to a known key via exact or partial match
        if track_key not in self.TRACK_NAME_MAPPING:
            for key in self.TRACK_NAME_MAPPING:
        ...
        if source.name == 'Racing News 365':
            race_slug = self.TRACK_SLUG_MAPPING.get(track_key)
            if not race_slug:
                return None
            return source.base_url + source.pattern.format(track=race_slug)

        elif source.name == 'Formula1.com':
            mapping = self.TRACK_NAME_MAPPING.get(track_key)
            if not mapping:
                return None
            race_id, url_slug = mapping
            # Auto-detect sprint qualifying for sprint-weekend tracks
            if session_type == 'auto':
                session_type = (
                    'sprint-qualifying'
                    if track_key in self.SPRINT_QUALIFYING_TRACKS
                    else 'qualifying'
                )
            return (
                source.base_url
    ...
    def _parse_qualifying_table(
        self, soup: BeautifulSoup, source: ScrapingSource
    ...
    def _clean_driver_name(self, name: str) -> str:
        ...
        # Handle "SURNAME First Name" format (e.g. "VERSTAPPEN Max")
        parts = name.split()
        ...
        for source_name in ['racingnews365', 'motorsport']:
            try:
                ...
                if results and len(results) >= 10:
                    ...
                    print(
                        f"📊 Top 3: P1-{results[0].driver_name}, "
                        f"P2-{results[1].driver_name}, P3-{results[2].driver_name}"
                    )
    ...
    def _inject_results_into_drivers(
        self, results: List[QualifyingResult], drivers: List
    ) -> List:
    ...
    def _names_match(self, name1: str, name2: str) -> bool:
        ...
        # First name + first letter of last name match
        if (len(name1_parts) >= 2 and len(name2_parts) >= 2
                and name1_parts[0] == name2_parts[0]
                and name1_parts[-1][0] == name2_parts[-1][0]):
            return True

        return False

    def get_f1com_url(self, track_name: str, session_type: str = 'auto') -> Optional[str]:
        """
        Return the F1.com results URL for a given track and session type.
        ...
        Args:
            ...
            session_type: 'qualifying', 'sprint-qualifying', or 'auto'.
        """
        return self._build_url(track_name, self.SOURCES['formula1'], session_type=session_type)
